...
class Collection implements \IteratorAggregate, \ArrayAccess, \Countable
{
     ...
     * @return $this
     */
    public static function createInstance(array $container = []): Collection
    {
        $instance = new static();
     ...
     * 
     * @return null|Collection Если `null`, ключ элемента коллекции не существует, 
     *     иначе новая коллекция.
     */
    public function factory(mixed $key): ?Collection
    {
        if (is_string($key)) {
     ...
     * 
     * @return null|array Если `null`, значение не найдено, иначе массив элементов 
    ...
    /**
     * Поиска параметра в коллекции по указанному значению и его имени.
     * 
     * Пример, поиск параметра с именем 'foobar' и значением 'name':
     * ```php
     * [
     *     'foobar' => ['name' => 'Name', ...],
     *     'fooBar' => 'Name',
     ...
     * ]
     * ```
     * Результат поиска: `['name' => 'Name', ...]`.
     * 
     * @param mixed $name Имя параметра коллекции.
     * @param mixed $value Значение параметра коллекции.
     * 
     * @return null|array Если `null`, параметра в коллекции не найден. Иначе, значение 
     *     параметра.
     */
    public function findAt(mixed $name, mixed $value): ?array
    {
     ...
     * @return mixed Если `null`, ключ элемента коллекции не существует.
     */
    public function &getFor(mixed $key): mixed
    {
        // чтобы не было: "Only variable references should be returned by reference"
        if (array_key_exists($key, $this->container))
            $_value = &$this->container[$key];
     ...
     * 
     * @param mixed $key Ключ элемента коллекции.
     * @param mixed $default Значение по умолчанию (значение по умолчанию `null`).
     * 
